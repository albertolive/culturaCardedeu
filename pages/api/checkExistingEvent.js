  ...
  try {
    ...
    if (!title || !startDate) {
      ...
      return res.status(400).json({
        error: "Missing required query parameters: title and startDate",
      });
    }
    ...
    try {
      ...
      return res.status(400).json({
        error: "Invalid startDate format. Please use a valid ISO date string.",
      });
    }

    try {
      const result = await getCalendarEvents({ from: fromDate, q: title });

      // getCalendarEvents returns an object with {events: [], noEventsFound: boolean}
      if (result && result.events && result.events.length > 0) {
        return res.status(200).json(result.events[0]);
      } else {
