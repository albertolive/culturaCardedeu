...
export async function complete(prompt, { cascade = "general", system, temperature = 0.1 } = {}) {
  ...
  // Bounded deadline: the gateway can take up to ~30s per provider attempt
  // while failing over internally, and a hung call would otherwise pin the
  ...
  if (res.status === 408 || res.status === 504) {
    throw new Error(`ai-gateway timed out after 60s`);
  }
