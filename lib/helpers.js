...
const fetchWeather = async () => {
  return await fetch('https://api.openweathermap.org/data/2.5/forecast?q=cardedeu&appid=583f803dfc6a7f2f96ff9957c330c2b0&units=metric&lang=ca').then(response => response.json()).then(data => {
  ...
  try {
    normalizedEvents = json?.items
      ? filterByDate
        ...
        : json.items.map((item) => normalizeItems(item, weather, normalizeRss))
      : [];
  } catch (e) {
    console.error(e);
    return [];
